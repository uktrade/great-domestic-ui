...
def test_full_name_with_user_details(user):
    user.first_name = "Joe"
    user.last_name = "Bloggs"
    assert user.full_name == "Joe Bloggs"


def test_full_name_with_company_details(user, company_profile):
    # This inherits the company_profile fixture with a postal_full_name
    # of Foo Example
    assert user.full_name == "Foo Example"


def test_full_name_with_no_details(user, company_profile):
