...
import { Store, ModerationState, ModerationStatus, ModerationErrorCode } from '../../state';
...
export default class ModerationBar extends React.Component<ModerationBarProps> {
    renderModal() {
        ...
        let onChangeComment = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
            e.preventDefault();
            this.props.store.dispatch(updateModerationState({comment: e.target.value}));
        ...
        let onSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            this.props.store.dispatch(updateModerationState({statusBoxOpen: false, submitStage: 'submitting'}));

            await this.props.api.submitModerationResponse(this.props.status, this.props.comment);

            // TODO handle error
            this.props.store.dispatch(updateModerationState({submitStage: 'submitted'}));
        };
        ...
        if (this.props.errors.has('comment-required')) {
            ...
            reasonErrors = <div className="error">This field is too long (200 characters maximum).</div>;
        }

        return (
            <div className="moderation-bar__modal">
                <div className="status" data-error={this.props.errors.has('status-required')}>
                    <p>Please select a status</p>
                    <input type="radio" id="approved" checked={this.props.status == 'approved'} onClick={setStatusOnClick('approved')}  />
                    <label htmlFor="approved">Approved</label>
                    <input type="radio" id="needs-changes" checked={this.props.status == 'needs-changes'} onClick={setStatusOnClick('needs-changes')} />
                    <label htmlFor="needs-changes">Needs changes</label>

                    {statusErrors}
                </div>

                <div className="reason" data-error={this.props.errors.has('status-required')}>
                    <p>Please give a reason for this status</p>

                    <textarea name="comment" value={this.props.comment} onChange={onChangeComment}></textarea>
                    <small>200 character limit.</small>

                    {reasonErrors}
                </div>

                <button className="btn" onClick={onSubmit}>Submit Review</button>
            </div>
        );
    }
    ...
    render() {
        let toggleStatusBox = (e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            this.props.store.dispatch(
                updateModerationState({statusBoxOpen: !this.props.statusBoxOpen})
            );
        };
        ...
        if (this.props.submitStage == 'not-submitted') {
            reviewButton = <>
                <span>Review</span>
                <button className="btn" onClick={toggleStatusBox}>+</button>
            </>;
        ...
        return (
            <div className="moderation-bar">
                ...
                <div className="moderation-bar__status">
                    {reviewButton}
                </div>
            </div>
        );
